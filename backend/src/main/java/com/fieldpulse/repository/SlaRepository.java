...
public interface SlaRepository extends JpaRepository<Sla, Long> {
    ...
    @Query("SELECT s FROM Sla s JOIN FETCH s.job j WHERE s.resolvedAt IS NULL AND j.status NOT IN (com.fieldpulse.domain.enums.JobStatus.COMPLETED, com.fieldpulse.domain.enums.JobStatus.CANCELLED)")
